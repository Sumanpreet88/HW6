...
class record
{
	...
	{
		ArrayList <stock> arr=new ArrayList<stock>();  //entering first three records
		arr.add(new stock());
		arr.add(new stock());
		arr.add(new stock());
		arr.get(0).setName("erty");
		arr.get(0).setSharePrice(2.30f, 4.50f);
        arr.get(0).setShares(40);
	    arr.get(1).setName("hgy");
		arr.get(1).setSharePrice(12.19f, 10.56f);
		arr.get(1).setShares(90);
	    arr.get(2).setName("hgfy");
		arr.get(2).setSharePrice(45.10f, 49.51f);
		arr.get(2).setShares(210);
		sorting(arr);             //sorting in increasing order of  company name
		...
		revsorting(arr);     //sorting in decreasing order of company name
		...
		{		System.out.println("enter the type of stock");
		       type=sc.nextLine();
		if(type.compareTo("simple")==0)    //if entered type in simple
		{
       ...
       //if entered type is foriegn
		...
		//getting lowest stock with highest and lowest value 
		...
		for(int i=0;i<arr.size();i++)
		{
			...
			//getting stock with highest and least profit
